...
public class Generator {
    ...
    public Grid generateGrid(Difficulty diff) {
        ...
        this.setNewGrid(new Grid(diff));
        this.solution.solveFor(getNewGrid()).findSolution(getNewGrid().getCellList(), 1);

        // Set the solution value for each cell
        for (Cell cell : getNewGrid()) {
        ...
        this.getNewGrid().provisionCells();
    ...
    public void emptyCells(int numRemove) {
        ...
        getNewGrid().provisionCells();

        // Execute for each cell in the grid
        while (getNewGrid().iterator().hasNext()) {

            // If the cell is not empty, empty it
            Cell cell = getNewGrid().iterator().next();
            ...
            this.solution.solveFor(getNewGrid());

            // Stop emptying cell if the specified number of empty cells is reached
            if (numRemove == allEmptyCells(getNewGrid()).size()) {
                break;
            } else {
                // If a unique solution can be produced with missing cells
                if (this.solution.findSolution(allEmptyCells(getNewGrid()), 3) != 1) {
                    getNewGrid().fetchCellProvision();
                } else {
                    cell.storeProvisionalValue();
                }
            }
     ...
     */
    private void setNewGrid(Grid newGrid) {
